...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        if (value < node.value) {
            node.leftChild = deleteElement(value, node.leftChild); // go through left
        } else if (value > node.value) {
            node.rightChild = deleteElement(value, node.leftChild); // go through right
        } else {
            if (node.leftChild == null && node.rightChild == null) {
                return null; // no children so leaf node
